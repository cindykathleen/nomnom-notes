    ...
    <div className="relative h-full w-screen p-16 mt-[80px] flex justify-center">
      <div className="max-w-[1440px] w-full px-8 flex flex-col space-y-8">
        ...
        <div className="flex gap-4">
          <input id="search-query" type="text" ref={searchQuery} className="min-w-md px-2 py-1 border border-charcoal border-solid rounded-sm focus:outline-none focus:border-darkpink focus:shadow-(--input-shadow)" autoComplete="off"></input>
          <button className="px-4 py-2 self-start text-snowwhite font-bold bg-darkpink rounded-lg cursor-pointer hover:bg-mauve transition-colors" onClick={handleSearch}>Search</button>
        </div>
        <div className="flex">
          <div className="flex flex-col min-w-3xl">
            {results.map((place) => {
              return (
                <div key={place._id} className="relative mb-8 bg-snowwhite cursor-pointer" onClick={() => { fetchLists(); setSelectedPlace(place); }}>
                  ...
                  <Link href={place.mapsUrl} target="_blank">
                    <div className="group flex gap-2">
                      ...
                      <p className="text-md hover:text-mauve transition-colors">Google Maps</p>
                    </div>
                  </Link>
                </div>
              );
