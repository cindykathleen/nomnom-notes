...
export default function ImageInput({ currImage, setNewImage }: { currImage: string, setNewImage: (newImage: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      {showUploadInput
        ? <label className="font-semibold">Image (<span className="text-darkpink cursor-pointer hover:text-mauve transition-colors" onClick={() => setShowUploadInput(false)} data-cy="image-input-type-trigger">use an existing image</span>)</label>
        : <label className="font-semibold">Image (<span className="text-darkpink cursor-pointer hover:text-mauve transition-colors" onClick={() => setShowUploadInput(true)} data-cy="image-input-type-trigger">upload your own</span>)</label>
      }
      {previewImage !== '' && (
        <img src={previewImage} className="max-w-36 mb-1 aspect-square object-contain" />
      )}
      {showUploadInput
        ? <input name="img-file" key="file-input" type="file" accept="image/*"
        ...
        : <input name="img-url" key="url-input" type="text" placeholder="Add image URL here"
